...
| 2f | TMC2209 UART not answering | 🚨 **LIVE.** All three report `Testing connection to TMC driver...failed! Likely cause: no power`. 🚨 **That message is ambiguous by construction** — `test_connection()` returns 2 whenever `DRV_STATUS` reads `0`, which happens both when the driver is unpowered **and** when it never replies at all. So it does **not** distinguish a missing 24 V supply from driver-slot jumpers not set for UART mode. Check both. |
